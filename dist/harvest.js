...
const stableId = (prefix, parts) => `${prefix}-${hash(parts.join("\0")).slice(0, 24)}`;
...
export function segmentBranch(branch, sessionId = branch[0]?.sessionId ?? "unknown") {
    const episodes = [];
    let current = [];
    const close = (closure) => {
        if (!current.length)
    ...
    for (const record of branch) {
        if (record.kind === "user" && !isCorrection(record.text))
            close("user-superseded");
        current.push(record);
        if (record.kind === "assistant")
            close("assistant-settled");
        else if (record.kind === "workflow" && record.receipt?.accepted === true)
            close("workflow-terminal");
    }
    close("incomplete");
    return episodes;
}
function makeEpisode(sessionId, records, closure) {
...
export function segmentSession(session) {
    ...
    for (const branch of session.branches) {
        for (const episode of segmentBranch(branch, session.sessionId)) {
            if (episode.records.some((record) => !activeIds.has(record.id)))
                continue;
...
export function successfulMutation(receipt) {
    return receipt?.mutation === true && receipt.isError !== true;
}
function isFailureReceipt(receipt) {
    return receipt?.validation === "failed" || receipt?.isError === true;
}
function hasOrderedFailureMutationSuccess(records) {
    let failedAt = -1;
    let mutationAt = -1;
    for (let index = 0; index < records.length; index++) {
        const receipt = records[index].receipt;
        if (receipt?.validation === "failed" && failedAt < 0)
            failedAt = index;
        if (failedAt >= 0 && index > failedAt && successfulMutation(receipt))
            mutationAt = index;
        if (receipt?.validation === "passed" && mutationAt >= 0 && index > mutationAt)
            return true;
    }
    return false;
}
function correctionSignal(records) {
    return records.some((record) => record.kind === "user" && isCorrection(record.text));
}
function acceptedCheckpoint(records) {
...
function decisionLanguage(record) {
...
function explicitDecision(records) {
...
function explicitProcedure(records) {
...
export const NOMINATING_SIGNALS = ["resolved-failure", "correction", "decision", "procedure"];
function evaluateSignals(records) {
    const signals = [];
    const reasons = [];
    const note = (signal, reason) => { signals.push(signal); reasons.push(reason); };
    if (hasOrderedFailureMutationSuccess(records))
        note("resolved-failure", "resolved-failure: failed validation, then successful mutation without error, then passed validation in one episode");
    if (correctionSignal(records))
        note("correction", "correction: user correction nominates review but does not establish truth");
    if (acceptedCheckpoint(records))
        note("workflow-checkpoint", "workflow-checkpoint: accepted checkpoint annotates the episode but cannot qualify it alone");
    if (explicitDecision(records))
        note("decision", "decision: explicit decision language corroborated by settlement evidence");
    if (explicitProcedure(records))
        note("procedure", "procedure: reusable procedure claim confirmed by successfully validated commands");
    return { signals, reasons };
}
export function detectHighSignal(records) {
    return evaluateSignals(records).signals;
}
export function explainSignals(records) {
...
export function shortlistEntries(episode, entries, limit = SHORTLIST_LIMIT) {
...
export function createPacket(episode, options) {
    if (!episode.hasNewRecord || !episode.eligible)
        return undefined;
    if (!episode.signals.some((signal) => NOMINATING_SIGNALS.includes(signal)))
        return undefined;
    if (episode.records.some((record) => record.origin === WORKER_ORIGIN))
        return undefined;
    ...
    if (!fit)
        return undefined; // the causal closure cannot fit; reject packet creation (guide 0.5)
...
function evidencePriority(record, index, records) {
    const receipt = record.receipt;
    if (receipt?.validation === "passed" || receipt?.accepted === true)
        return 1;
    const earlierFailure = records.slice(0, index).some((other) => isFailureReceipt(other.receipt));
    if (successfulMutation(receipt) && earlierFailure)
        return 2;
    if (record.kind === "user" && isCorrection(record.text))
        return 2;
    if (isFailureReceipt(receipt)) {
        const laterRecovery = records.slice(index + 1).some((other) => successfulMutation(other.receipt) || other.receipt?.validation === "passed" || other.receipt?.accepted === true);
        return laterRecovery ? 3 : 6;
    }
    if (decisionLanguage(record))
        return 4;
    if (receipt)
        return 5;
    return 6;
}
const overCap = (packet, cap) => JSON.stringify(packet).length > cap;
...
function fitPacket(packet, cap, priorities) {
    let omitted = 0;
    const dropAt = (index) => { packet.evidence.splice(index, 1); priorities.splice(index, 1); omitted++; };
    const seen = new Set();
    ...
    for (const limit of [300, 80]) {
        for (let index = 0; index < packet.evidence.length && overCap(packet, cap); index++) {
            if (priorities[index] === 6)
                packet.evidence[index].excerpt = cleanExcerpt(packet.evidence[index].excerpt, limit);
        }
    }
    for (const level of [6, 5, 4, 3, 2]) {
    ...
    while (overCap(packet, cap) && packet.shortlist.length) {
        ...
        if (item.summary.length > 120)
            item.summary = cleanExcerpt(item.summary, 120);
        else
            packet.shortlist.pop();
    }
