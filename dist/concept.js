...
function rejectReserved(value, field, issues) {
...
function normalizeList(value, field, issues) {
    if (value === undefined)
        return [];
    ...
    value.forEach((item, index) => {
        ...
        if (itemText && !result.includes(itemText))
            result.push(itemText);
    });
    return result.sort((a, b) => (a < b ? -1 : a > b ? 1 : 0));
}
function optionalDate(value, issues) {
...
function optionalKind(value, issues) {
    if (value === undefined || value === "")
        return undefined;
    if (value === "gotcha" || value === "decision" || value === "procedure" || value === "invariant")
        return value;
...
export function validateEntry(value) {
    const issues = [];
    if (!isRecord(value))
        throw new KnowledgeValidationError(["entry must be a mapping"]);
    ...
    for (const [field, text] of [["id", id], ["title", title], ["summary", summary], ["body", body]]) {
        if (text)
            rejectReserved(text, field, issues);
    }
    ...
    const entry = { id, title, summary, body: normalizeMultiline(body) };
    if (date)
        entry.date = date;
    if (tags.length > 0)
        entry.tags = tags;
    ...
    if (verificationSources.length > 0)
        entry.verificationSources = verificationSources;
    return entry;
}
function metadataFor(entry) {
    const metadata = { id: entry.id, title: entry.title, summary: entry.summary };
    if (entry.date)
    ...
    if (entry.verifiedAt)
        metadata.verifiedAt = entry.verifiedAt;
    if (entry.verificationSources && entry.verificationSources.length > 0)
        metadata.verificationSources = entry.verificationSources;
    return metadata;
}
...
export function renderKnowledgeDocument(document) {
...
export function parseKnowledgeDocument(markdown) {
    ...
    for (;;) {
        ...
        if (firstOpen < 0)
            ...
            gaps.push(normalized.slice(cursor, open));
        const metadataStart = open + OPEN_MARKER.length;
        const metadataEnd = normalized.indexOf("-->", metadataStart);
        if (metadataEnd < 0)
            throw new KnowledgeValidationError(["entry metadata must be closed with -->"]);
        let raw;
        ...
        if (!isRecord(raw))
            throw new KnowledgeValidationError(["entry metadata must be a JSON object"]);
        const blockStart = metadataEnd + 3;
        const blockEnd = normalized.indexOf(CLOSE_MARKER, blockStart);
        if (blockEnd < 0)
            throw new KnowledgeValidationError(["entry must be closed with a closing comment"]);
        ...
        if (ids.has(entry.id))
            throw new KnowledgeValidationError([`duplicate entry id: ${entry.id}`]);
        ids.add(entry.id);
        entries.push(entry);
...
export function removeEntriesFromSessions(entries, sessionIds) {
    const excluded = new Set(sessionIds);
    if (excluded.size === 0)
        return { entries: [...entries], removed: 0 };
...
export function applyCuratedEntry(entries, input, projectKey) {
    ...
    if (input.action === "update") {
        if (!input.targetEntryId)
            throw new KnowledgeValidationError(["update requires targetEntryId"]);
        const index = entries.findIndex((entry) => entry.id === input.targetEntryId);
        if (index < 0)
            throw new KnowledgeValidationError([`update target not found: ${input.targetEntryId}`]);
        ...
        if (!changed)
            return { entries: [...entries], changed, id: existing.id };
    ...
    if (index >= 0) {
        ...
        if (!changed)
            return { entries: [...entries], changed, id };
        const updated = entries.slice();
...
const DIGEST_FIELDS = ["title", "summary", "body", "date", "tags", "sources", "kind", "verifiedAt", "verificationSources"];
...
export function entryDigest(entry) {
    ...
    for (const field of DIGEST_FIELDS)
        content[field] = stableValue(normalized[field]);
    return createHash("sha256").update(JSON.stringify(content)).digest("hex");
}
export function corpusRevision(entries) {
