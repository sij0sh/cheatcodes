...
export function searchKnowledgeTool(env = process.env) {
    return defineTool({
        ...
        execute: async (_toolCallId, params) => {
            ...
            if (modes.length === 0)
                return text(TOOL_USAGE, true);
            if (modes.length > 1)
                return text(`error: ${modes.join(" and ")} cannot combine. ${TOOL_USAGE}`, true);
            switch (modes[0]) {
...
async function searchCorpus(root, env, query, limit) {
...
async function loadEpisode(root, manifestId, packetId) {
    const manifest = await readManifest(root, manifestId);
    if (!manifest)
        return text(`error: unknown manifest ${manifestId}`, true);
    ...
    if (!packet)
        return text(`error: manifest ${manifestId} has no packet ${packetId}`, true);
    return text({
...
async function inspectFact(root, filePath, pattern, maxLines) {
    ...
    try {
        ...
        return text(`error: cannot read ${filePath}`, true);
    }
...
async function inspectTree(root) {
    ...
    const render = (list, truncated) => JSON.stringify({ root: inventory.root, totalFiles: inventory.totalFiles, truncated, entries: list });
    let list = entries;
