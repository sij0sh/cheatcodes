...
export const EMPTY_PROJECT_STATE = { files: {} };
export const EMPTY_GLOBAL_STATE = { version: 1, projects: {} };
...
export function orderState(state) {
    ...
    for (const key of Object.keys(state.projects).sort()) {
        ...
        const files = {};
        for (const file of Object.keys(project.files).sort())
            files[file] = project.files[file];
        const ordered = { files };
        if (project.lastRun)
            ordered.lastRun = project.lastRun;
...
function validateCursor(value, source) {
    ...
    if (typeof raw.sessionId !== "string" || !raw.sessionId)
        throw new Error(`${source}.sessionId must be a non-empty string`);
    if (typeof raw.prefixSha256 !== "string")
        throw new Error(`${source}.prefixSha256 must be a string`);
    ...
    return {
        ...
        committedOffset: raw.committedOffset,
        observedSize: raw.observedSize,
        mtimeMs: raw.mtimeMs,
        prefixSha256: raw.prefixSha256,
    };
}
const OUTCOMES = new Set(["success", "failed", "coalesced", "timeout"]);
function validateRunRecord(value, source) {
    const raw = requireObject(value, `${source} must be an object`);
    if (raw.version !== 1)
    ...
    if (typeof raw.pid !== "number" || !Number.isInteger(raw.pid))
        throw new Error(`${source}.pid must be an integer`);
    if (typeof raw.outcome !== "string" || !OUTCOMES.has(raw.outcome))
        throw new Error(`${source}.outcome must be one of success, failed, coalesced, timeout`);
    const record = {
        version: 1,
    ...
    if (raw.reason !== undefined) {
        if (typeof raw.reason !== "string")
            throw new Error(`${source}.reason must be a string`);
        record.reason = raw.reason;
    }
    for (const field of ["changedFiles", "curatorCalls", "entriesWritten"]) {
    ...
    if (raw.warnings !== undefined) {
        if (!Array.isArray(raw.warnings) || !raw.warnings.every((item) => typeof item === "string"))
            throw new Error(`${source}.warnings must be a list of strings`);
...
export async function loadGlobalState(env = process.env) {
    ...
    try {
        ...
        if (error.code === "ENOENT")
            return { ...EMPTY_GLOBAL_STATE, projects: {} };
        throw error;
    }
    let raw;
    try {
    ...
    for (const [key, value] of Object.entries(projectsRaw)) {
        ...
        const files = {};
        for (const [name, cursor] of Object.entries(filesRaw))
            files[name] = validateCursor(cursor, `${file}.projects.${key}.files.${name}`);
        const state = { files };
        if (project.lastRun !== undefined)
            state.lastRun = validateRunRecord(project.lastRun, `${file}.projects.${key}.lastRun`);
        projects[key] = state;
    }
    return { version: 1, projects };
}
...
async function acquireLock(file, options = {}) {
    ...
    for (;;) {
        try {
            ...
            let ownerPid = 0;
            try {
                ownerPid = JSON.parse(await readFile(file, "utf8")).pid ?? 0;
            }
            catch {
                ownerPid = 0;
            }
            ...
            if (!options.coalesce)
                throw new Error(`Another cheatcodes run is active (lock: ${file})`);
            coalesced = true;
            break;
        }
    }
...
export function stateLockPath(env = process.env) {
    return path.join(path.dirname(globalStatePath(env)), "state.lock");
}
export function projectLockPath(env = process.env, projectKey) {
    const safe = projectKey.replace(/[^A-Za-z0-9._-]/g, "_");
    return path.join(path.dirname(globalStatePath(env)), "locks", `${safe}.lock`);
}
export async function acquireStateLock(env = process.env) {
    return acquireLock(stateLockPath(env), { waitMs: 10_000 });
}
export async function acquireProjectLock(env, projectKey, options = {}) {
    return acquireLock(projectLockPath(env, projectKey), options);
}
export async function updateProjectState(env, projectKey, mutate) {
