...
import { checkMapFreshness, runMap } from "./map.js";
...
function defaultCurate(context) {
...
async function defaultSyncWorkflow(context) {
    ...
    if (!result.started)
        return { outcome: "none" };
    if (result.terminal?.status !== "completed")
        return { outcome: "parked", warning: result.warning ?? `workflow ended ${result.terminal?.status ?? "unknown"}`, warnings: result.warnings };
    if (!result.applied)
        return { outcome: "parked", warning: result.warning ?? "workflow completed without committing", warnings: result.warnings };
    return { outcome: "completed", warnings: result.warnings };
}
function defaultSynthesizeMap(context) {
    return runMap({ root: context.root, env: context.env }).then((result) => ({
        ok: result.status === "committed",
        warning: result.warning,
    }));
}
export function resolveEnsureTimeoutSeconds(env, override) {
...
export async function runEnsure(options = {}) {
    ...
    const workflow = curate.outcome === "timeout"
        ? { outcome: "skipped", warning: "curation timed out" }
        : await stages.syncWorkflow(context);
    if (workflow.warning)
        context.warnings.push(workflow.warning);
    ...
    try {
        ...
        if (freshness.state === "stale")
            map = freshness.reason === "sources changed" ? "stale (sources changed)" : "stale (inventory changed)";
        else if (freshness.state === "absent")
            map = "absent";
        else
            map = "fresh";
    }
    catch (error) {
        return { status: "error", warning: `map freshness check failed: ${error.message}`, warnings: context.warnings };
    }
    ...
    if (curate.outcome === "timeout" || workflow.outcome === "skipped") {
        ...
        if (curate.outcome === "timeout")
            result.warning = "curation hit its deadline";
    }
        ...
        if (curate.changedFiles > 0 || workflow.outcome === "completed" || map === "synthesized")
