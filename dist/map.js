...
export function isMapEntry(entry) {
    return (entry.tags ?? []).some((tag) => MAP_FAMILIES.includes(tag));
}
export function validateMapOperations(value, context) {
    ...
    for (const [index, op] of parsed.operations.entries()) {
        ...
        if (tags.length !== 1 || !MAP_FAMILIES.includes(tags[0])) {
...
export function planMapRetirements(existing, submitted, now = new Date()) {
    const kept = new Set();
    for (const op of submitted) {
        if (op.op === "create" || op.op === "update")
            kept.add(normalizeTitleKey(op.entry.title));
    }
    return existing
...
export function describeMapOperations(operations, projectKey) {
    return operations.map((op) => {
        ...
        if (op.op === "delete")
            return `delete ${op.target.id} (${op.reason})`;
        return op.op;
    });
}
