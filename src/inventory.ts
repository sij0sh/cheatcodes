...
async function walk(root: string, relative: string, depth: number, state: WalkState): Promise<boolean> {
  ...
  for (const dirent of dirents) {
    if (state.entries.length >= TREE_LIMITS.entries) return true;
    if (dirent.isSymbolicLink()) continue;
    const rel = relative ? `${relative}/${dirent.name}` : dirent.name;
    if (dirent.isDirectory()) {
      if (SKIP_DIRS.has(dirent.name)) continue;
      state.entries.push({ path: `${rel}/` });
      if (await walk(root, rel, depth + 1, state)) return true;
...
export async function walkInventory(root: string): Promise<Inventory> {
  const rootReal = await realpath(root).catch(() => root);
  const state: WalkState = { entries: [], files: { count: 0 } };
...
export async function inventoryDigest(root: string): Promise<string> {
