...
import { entryDigest, normalizeTitleKey, type KnowledgeEntry } from "./concept.js";
...
function bodyTerms(body: string): Set<string> {
  return new Set(
    body
      .toLowerCase()
      .split(/[^a-z0-9_.-]+/)
...
function configValues(body: string): Map<string, Set<string>> {
...
function pairReasons(a: KnowledgeEntry, b: KnowledgeEntry): string[] {
  ...
  if (titleOverlap >= 0.6 || contained) reasons.push("similar titles");
  ...
  if (sharedSources.length > 0) reasons.push("shared sources");
  if (jaccard(bodyTerms(a.body), bodyTerms(b.body)) >= 0.5) reasons.push("overlapping bodies");
...
function clusterId(entryIds: readonly string[]): string {
  return `cl-${createHash("sha256").update(entryIds.join("\u0000")).digest("hex").slice(0, 12)}`;
}

export function clusterCandidates(entries: readonly KnowledgeEntry[]): CandidateCluster[] {
  ...
  for (let i = 0; i < sorted.length; i++) {
    for (let j = i + 1; j < sorted.length; j++) {
      ...
      for (const reason of reasons) if (!known.includes(reason)) known.push(reason);
...
export function digestFor(entries: readonly KnowledgeEntry[], id: string): string | undefined {
  const entry = entries.find((candidate) => candidate.id === id);
  return entry ? entryDigest(entry) : undefined;
}

function isVerified(entry: KnowledgeEntry): boolean {
  return entry.verifiedAt !== undefined && (entry.verificationSources ?? []).length > 0;
}

export function pickSurvivor(entries: readonly KnowledgeEntry[], ids: readonly string[]): KnowledgeEntry {
...
export function proposeOperations(clusters: readonly CandidateCluster[], entries: readonly KnowledgeEntry[]): KnowledgeOperation[] {
